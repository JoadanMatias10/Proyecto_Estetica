...
export default function DetalleProductoCliente() {
  ...
  if (!prod || errorType === "not_found") {
    return (
      <div className="card p-8 text-center min-h-[50vh] flex flex-col justify-center items-center">
        ...
        <Link to="/cliente/productos" className="text-violet-600 hover:text-violet-700 font-medium underline">Volver al catalogo</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link to="/cliente/productos" className="text-violet-600 hover:text-violet-700 text-sm font-medium transition-colors mb-8 inline-flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center mt-4 pb-8 border-b border-slate-100">
            <div className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl w-full sm:w-40 h-14 px-4 shadow-sm">
              <button
                onClick={() => handleCantidad(cantidad - 1)}
                className="text-violet-400 hover:text-violet-600 font-bold text-xl px-2 transition-colors"
              >-</button>
              <span className="text-slate-800 font-bold text-lg">{cantidad}</span>
              <button
                onClick={() => handleCantidad(cantidad + 1)}
                className="text-violet-400 hover:text-violet-600 font-bold text-xl px-2 transition-colors"
              >+</button>
